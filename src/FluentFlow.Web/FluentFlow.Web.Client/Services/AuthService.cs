    ...
    NavigationManager nav)
{
    public async Task<(bool Success, string? Error)> LoginAsync(LoginDto dto)
    {
        ...
        if (result is null) return (false, "Email ou password incorrectos.");

        await authStateProvider.SetAuthAsync(result);
        return (true, null);
    }
    ...
    public async Task<(bool Success, string? Error)> RegisterAsync(RegisterDto dto)
    {
        ...
        if (result is null) return (false, "Erro ao criar conta. Verifique os dados.");
